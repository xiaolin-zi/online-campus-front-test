...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // 用户模块
      '/user-api': {
        target: 'http://117.72.15.203:7001/campusUser/',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/user-api/, '')
      },
      // 消息模块
      '/message-api': {
        target: 'http://117.72.15.203:7002/campusMessage/',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/message-api/, '')
      },
      // 动态模块
      '/contact-api': {
        target: 'http://117.72.15.203:7003/campusContact/',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/contact-api/, '')
      },
      // 交易模块
      '/trade-api': {
        target: 'http://117.72.15.203:7004/campusTrade/',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/trade-api/, '')
      },
      // oss模块 (文件上传与下载)
      '/oss-api': {
        target: 'http://117.72.15.203:7005/campusOss/',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/oss-api/, '')
      },
      // 兼职模块
      '/parttime-api': {
        target: 'http://117.72.15.203:7006/campusParttime/',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/parttime-api/, '')
      },
      // 招募模块
      '/recruit-api': {
        target: 'http://117.72.15.203:7007/campusRecruit/',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/recruit-api/, '')
      },
    }
